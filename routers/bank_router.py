...
from services.bank_service import create_bank_account_service, transfer_money_service
...
async def transfer_money(
    ...
    result = transfer_money_service(current_user["phone"], receiver_phone, amount)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return result